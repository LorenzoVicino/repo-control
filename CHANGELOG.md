...
- Rebuild the dashboard home as a grid of widgets you can arrange. The page had grown into a ledger of every repository, a signals ring, a change chart, runtime and commit panels and a quotation - a compressed copy of the section pages rather than a place to start from. It is now a resume-and-triage surface: **Needs attention** leads with one queue ordered by severity (unhealthy containers, automations that did not finish, repositories to pull, commit or push), **Workspace** draws the repository states as a ring with the count in its hole and, at medium size, the change-concentration bars, **Docker runtime** shows running containers and health per group, **Pick up where you left off** lists the repositories opened most recently, then favorites, then the newest commits, **AI conversations** shows the latest Codex, Claude Code and Gemini CLI sessions with a Resume button and a fourteen-day activity chart, **Automations** shows active and recent runs with a duration chart coloured by outcome, and **Shortcuts** keeps the three actions that otherwise live only on a key. A **Favorites** widget exists and starts hidden, since favorites have a section of their own. **Customize** enters an explicit edit mode in which widgets are dragged into place, moved with keyboard buttons, set to small, medium or large, hidden and shown again, or the default layout restored; outside it nothing moves by accident. The arrangement and the recently opened repositories are stored in the server-side `preferences.json` next to the favorites, so they follow the workspace rather than the browser.
...
- Serve an `llms.txt` at the dashboard root, following the llmstxt.org format: an H1, a blockquote summary, then H2-delimited link lists pointing at the README, architecture guide, security policy and changelog. The single-page fallback answers every unmatched GET with `index.html`, so before this the path returned the application shell with a 200 — which an audit reads as a malformed `llms.txt` rather than an absent one. The summary is written for the case that actually applies here: repo-control is not a published corpus, every `/api` path is an operation on the operator's real repositories, and a GET is not automatically safe to repeat.
...
- Re-tokenise the five colour palettes. They shared one subdued structure and told apart mostly by a tint, so the choice read as five greys. Each now has a ground of its own and a three-step accent ramp: **White** is paper with a deep indigo, **Black** graphite with a luminous violet, **Red** warm charcoal with an ember coral, **Blue** deep navy with a sky blue, **Green** forest with a mint. The status inks (success, warning, error, info) were re-picked with them. Every text ink clears 4.5:1 on the highest surface it lands on, the tertiary ink stays below the secondary so the three-step ramp keeps its order, and each accent ramp validates as an ordinal scale, which the change-concentration chart relies on. The selected toggle label uses the dark accent step on the light palette, where the light step fell under 4.5:1 against its tint.
- Keep the ring interaction the previous panel had: pointing at an arc, or hovering or focusing its legend row, thickens that arc, dims the others and shows that state's count in the hole instead of the total. A state at zero draws no arc and its legend row stays inert.
...
- Make the signals ring answer per state. Pointing at a segment, or at its legend row, thickens that arc, dims the others and turns the hole into that state's exact count, so a single number no longer needs a second place to look. A state at zero draws no arc, so its legend row stays inert rather than dimming the ring for nothing. The change concentration chart beside it is unchanged.
